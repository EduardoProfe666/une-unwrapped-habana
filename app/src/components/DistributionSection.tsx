        ...
        <section className="bg-white border-4 border-black p-8 shadow-[12px_12px_0px_0px_black] overflow-hidden">
            <header className="mb-10 border-b-4 border-black pb-4 flex justify-between items-end">
                <div>
                    ...
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Distribución de
                        contenido y sentimiento</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
                <div className="lg:col-span-3 space-y-6">
                    <h3 className="text-xs font-black bg-black text-white px-2 py-1 w-fit mb-4 uppercase">Frecuencia por
                        Categoría</h3>
                        ...
                        {messageDistribution.map(item => (
                            <div key={item.typeId} className="group relative">
                                ...
                                <div className="h-8 border-2 border-black bg-gray-50 relative overflow-hidden shadow-[3px_3px_0px_0px_black]">
                                    <div className={`absolute top-0 left-0 h-full border-r-2 border-black transition-all duration-1000 ease-out ${primaryColorClass} z-10`} style={{width: `${item.percentage}%`}}/>
                                ...
                                <div className="absolute top-0 left-0 w-full h-full opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[30] flex items-center justify-center">
                                    <div
                                        className="bg-black text-white p-2 text-[10px] font-bold border-2 border-white translate-y-8 shadow-xl max-w-[200px] text-center uppercase tracking-tighter">
                                        {item.description}
                                    </div>
                                </div>
                            </div>
                ...
                <div
                    className="lg:col-span-2 flex flex-col bg-gray-50 border-4 border-black p-6 shadow-[6px_6px_0px_0px_black]">
                    <h3 className="text-xs font-black uppercase border-b-2 border-black pb-2 mb-8 text-center">Balance
                        de Reacciones</h3>
                    ...
                    <div className="mt-8 text-center border-t-2 border-black pt-4">
                        <span
                            className="text-[10px] font-black text-gray-400 uppercase">Muestra Total: {totalReactions.toLocaleString()}</span>
                    </div>
                </div>
            </div>
        </section>
