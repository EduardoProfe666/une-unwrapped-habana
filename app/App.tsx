...
function App() {
  ...
  if (!data) return <div className="min-h-screen flex items-center justify-center font-bold text-2xl">Cargando...</div>;

  return (
    <div className={`min-h-screen ${theme.bg} text-black transition-colors duration-500 relative`}>
      
      {/* Sync Date Corner */}
      <div className="fixed top-0 right-0 p-2 z-50 bg-black text-white text-[10px] font-mono opacity-60 hover:opacity-100 transition-opacity">
      ...
      <header className="min-h-[80vh] flex flex-col items-center justify-center p-6 relative overflow-hidden border-b-4 border-black bg-white">
         <div className="absolute top-4 left-4 font-bold text-xl flex items-center gap-2">
            <img src="/logo.png" alt="Logo" className="h-10 w-10 p-0.5 rounded-full bg-gray-300 border-2 border-black" />
            UNE Unwrapped
         </div>
         ...
         {/* Hero Decor */}
         <div className={`absolute -bottom-20 -left-20 w-64 h-64 rounded-full ${theme.secondary} opacity-50 blur-3xl`}></div>
         <div className={`absolute top-20 right-20 w-40 h-40 rounded-full ${theme.primary} opacity-30 blur-2xl`}></div>
      </header>

      {/* Disclaimer Section */}
      <section className="bg-yellow-300 border-b-4 border-black p-4 text-center text-sm font-bold flex justify-center items-center gap-2">
         <HelpCircle size={18} />
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-24">
        ...
        >
            {[
                ...
                <motion.div key={i} variants={itemVariants} className="bg-white neobrutal-border p-4 neobrutal-shadow flex flex-col justify-between h-32">
                    <div className="flex justify-between items-start text-gray-500">
                        <span className="text-xs font-bold uppercase">{stat.label}</span>
                        {stat.icon}
                    </div>
                    <span className="text-2xl md:text-4xl font-black truncate" title={stat.value.toLocaleString()}>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            >
                ...
                <ul className="space-y-3 font-mono text-lg">
                    <li className="flex justify-between"><span>Vistas/Msg:</span> <b>{formatNumber(data.avg_views)}</b></li>
                    <li className="flex justify-between"><span>Reacciones/Msg:</span> <b>{data.avg_reactions}</b></li>
                    <li className="flex justify-between"><span>Longitud Texto:</span> <b>{data.avg_text_length} letras</b></li>
                    <li className="flex justify-between text-green-600"><span>👍 Avg:</span> <b>{data.avg_positive_reactions}</b></li>
                    <li className="flex justify-between text-red-600"><span>👎 Avg:</span> <b>{data.avg_negative_reactions}</b></li>
                </ul>
            </motion.div>
            ...
            >
                 <div className="relative group">
                    <span className="absolute -rotate-3 -top-3 left-4 bg-black text-white px-2 text-xs font-bold z-20">PRIMER MENSAJE</span>
                    <TelegramMessage message={data.first_message} />
                 </div>
                 <div className="relative group mt-12">
                    <span className="absolute -rotate-3 -top-3 left-4 bg-black text-white px-2 text-xs font-bold z-20">ÚLTIMO MENSAJE</span>
                ...
                <div className="space-y-4">
                    {Object.entries(data.distribution_message).map(([typeId, count]) => (
                        <div key={typeId} className="flex items-center gap-2 group cursor-help relative">
                            <div className="w-full bg-gray-100 h-10 rounded-none overflow-hidden relative border-2 border-black shadow-[2px_2px_0px_0px_black]">
                                <div 
                                    className={`h-full ${theme.primary} absolute top-0 left-0`} 
                                    style={{ width: `${((count as number) / data.total_messages) * 100}%` }}
                                ></div>
                                <span className="absolute left-2 top-1/2 -translate-y-1/2 z-10 font-bold text-xs md:text-sm mix-blend-multiply uppercase">
                            ...
                            <div className="hidden group-hover:flex absolute left-0 bottom-full bg-black text-white p-3 text-xs w-64 z-20 mb-2 border-2 border-white shadow-lg pointer-events-none">
                ...
                <div className="flex flex-col items-center justify-center bg-gray-50 border-2 border-dashed border-gray-400 p-4">
                    ...
                    <div className="flex gap-12 items-end h-64 w-full justify-center px-4">
                         <div className="flex flex-col items-center flex-1 h-full justify-end group">
                            <span className="font-black text-2xl mb-2">{((data.total_positive_reactions / data.total_reactions) * 100).toFixed(1)}%</span>
                            <div 
                         ...
                         <div className="flex flex-col items-center flex-1 h-full justify-end group">
                            <span className="font-black text-2xl mb-2">{((data.total_negative_reactions / data.total_reactions) * 100).toFixed(1)}%</span>
                            <div 
        ...
        {[
            ...
            { title: "Top 3: Más Comentados", items: data.top3_most_replied_messages, color: "bg-yellow-200" },
            { title: "Top 3: Más Negativos 🤬", items: data.top3_most_negative_reaction_messages, color: "bg-red-200" },
            { title: "Top 3: Más Positivos 😍", items: data.top3_most_positive_reaction_messages, color: "bg-green-200" },
        ].map((section, idx) => (
      ...
      <footer className="bg-black text-white p-8 mt-24 text-center border-t-4 border-gray-800">
         ...
         <p className="text-sm text-gray-400 max-w-2xl mx-auto mb-8">
            ...
            Los datos son una aproximación basada en mensajes públicos de Telegram y pueden contener errores de interpretación algorítmica.
         </p>
         <div className="flex justify-center items-center gap-4">
            <a href="https://t.me/EmpresaElectricaDeLaHabana" target="_blank" rel="noreferrer" className="bg-white text-black px-4 py-2 font-bold rounded hover:bg-gray-200">
                Canal Oficial
            </a>
            {/* Hidden Feature */}
            <a href="https://eduardoprofe666.github.io" className="opacity-30 hover:opacity-100 transition-opacity text-2xl" title="???">
                🎩
            </a>
         </div>
      </footer>
